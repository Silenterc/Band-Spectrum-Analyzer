...
void SignalSlotModeButton::mouseDown(const juce::MouseEvent &) {
    if (onPress)
        onPress();
}

void SignalSlotModeButton::mouseEnter(const juce::MouseEvent &) {
    hovered = true;
    repaint();
}

void SignalSlotModeButton::mouseExit(const juce::MouseEvent &) {
    hovered = false;
    repaint();
}
