...
public:
    struct Callbacks {
        std::function<void(const PluginPresets::PresetId&)> onLoadRequested;
        std::function<void(const PluginPresets::PresetDescriptor&, juce::Rectangle<int>)> onDeleteRequested;
    };
    ...
    [[nodiscard]] const PluginPresets::PresetId& getPresetId() const noexcept { return descriptor.id; }
...
private:
    ...
    PluginPresets::PresetDescriptor descriptor;
    Callbacks callbacks;
