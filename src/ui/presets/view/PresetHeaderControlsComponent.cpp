...
void PresetHeaderControlsComponent::presetUiSnapshotChanged(const Ui::Presets::PresetUiSnapshot& snapshot) {
...
void PresetHeaderControlsComponent::updateButtonState() {
    ...
    const auto shouldShowDirtyMarker
        = presetUiSnapshot.selectionStatus == Ui::Presets::PresetSelectionStatus::selectedDirty;
...
void PresetHeaderControlsComponent::launchCallout(std::unique_ptr<juce::Component> content,
    ...
    calloutPresenter.launch(std::move(content), makeCalloutAnchorBounds(triggerComponent), *parentComponent);
...
void PresetHeaderControlsComponent::showBrowserPopup() {
    ...
    auto content = std::make_unique<PresetBrowserPopupComponent>(
        theme,
        presetUiSnapshot,
        PresetBrowserPopupComponent::Callbacks{
            [safeThis](const Ui::Presets::PresetId& presetId) {
                if (safeThis == nullptr)
...
void PresetHeaderControlsComponent::showSavePopup() {
    ...
    auto content = std::make_unique<PresetSavePopupContent>(
        ...
        PresetSavePopupContent::Callbacks{
            ...
            [safeThis](const Ui::Presets::PresetId& presetId, const juce::String& name) {
                if (safeThis == nullptr)
...
void PresetHeaderControlsComponent::focusPopupContent(juce::Component* content) const {
    ...
    if (auto* browserPopup = dynamic_cast<PresetBrowserPopupComponent*>(content)) {
        browserPopup->focusInitialControl();
        return;
    }
...
juce::Component* PresetHeaderControlsComponent::getCalloutParentComponent() const {
    return Ui::findCalloutParentComponent(*this);
}
...
juce::Rectangle<int> PresetHeaderControlsComponent::makeCalloutAnchorBounds(juce::Component& triggerComponent) const {
    ...
    return Ui::makePointCalloutAnchor(parentComponent->getLocalArea(this, triggerComponent.getBounds()), true);
}
